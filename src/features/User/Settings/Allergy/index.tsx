...
import { UserAllergy, Allergy } from "./type";
import useAuth from "@/features/hooks/useAuth";

export const AllergySetting = ({ allergies }: { allergies: UserAllergy[] }) => {
  //ユーザーが持っているアレルギーの一覧
  const user = useAuth();
  if (user) {
    //ユーザーIDの取得方法を要チェック
    const userId = user.uid;
    const [userAllergiesList, setUserAllergiesList] = useState<UserAllergy[]>(allergies);

    //アレルギーIDとアレルギー名を取り出しただけのリスト
    const [allergiesList, setAllergies] = useState<Allergy[]>(
      allergies.map((allergy) => {
        if (allergy.UserId === userId) {
          return allergy.UserAllergyId;
        }
      }),
    );
    //チェックボックスをチェックした時のアレルギーリスト
